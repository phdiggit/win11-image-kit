...
30. Issue #16 构建、捕获、部署和验收证据链报告见 [Evidence Chain Report](docs/48-issue16-evidence-chain-report.md)、[Evidence Chain Acceptance](docs/49-issue16-evidence-chain-acceptance.md)、[Close Preparation](docs/50-issue16-close-preparation.md)、[Main Validation Evidence](docs/51-issue16-main-validation-evidence.md)；post-PR #84 main Full Validate ready evidence 已记录
...
Issue #18 version-aware default apps / Start menu restore intake and report-only acceptance scaffold 见 [User Experience Restore Intake](docs/58-issue18-user-experience-restore-intake.md) 和 [User Experience Restore Acceptance](docs/59-issue18-user-experience-restore-acceptance.md)；capability matrix、template metadata、scope semantics 和 verification plan 见 [User Experience Capability Matrix](docs/60-issue18-user-experience-capability-matrix.md)，Restore-UserExperience handler integration 见 [Restore Handler Integration](docs/61-issue18-restore-handler-integration.md)，当前 report-only / handler-adapter stage 的人工收口准备和 post-PR #96 main/workflow ready evidence 见 [Close Preparation](docs/62-issue18-close-preparation.md) 与 [Main Validation Evidence](docs/63-issue18-main-validation-evidence.md)。维护者人工关闭交接见 [Manual Closure Handoff](docs/64-issue18-manual-closure-handoff.md)，未来真实执行拆分见 [Future True UX Restore Execution Split](docs/65-future-true-ux-restore-execution-split.md)，未来真实执行授权入口、证据模型和 dry-run plan 见 [Authorization Intake](docs/66-future-true-ux-restore-authorization-intake.md)、[Evidence Model](docs/67-future-true-ux-restore-evidence-model.md) 与 [Dry-run Plan](docs/68-future-true-ux-restore-dry-run-plan.md)。当前仍为 report-only / authorization-intake；PR Fast CI、fixture、handler report、authorization report 和 manual checklist 不是真实 UX restore evidence，真实 UX restore execution 仍需要后续明确授权。
