Describe "Controlled execution validation runner" {
    BeforeEach {
        ...
        $script:InvokeControlledExecutionProcess = {
            ...
            $psi.Arguments = "-NoProfile -ExecutionPolicy Bypass -File `"$script:RepoRoot\scripts\validate\Test-ControlledExecution.ps1`" -ManifestPath `"$ManifestPath`" -ReportPath `"$ReportPath`""
