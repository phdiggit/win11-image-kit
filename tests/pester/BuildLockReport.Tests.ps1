Describe "Build lock report" {
    BeforeEach {
        ...
        $script:InvokeTestBuildLockProcess = {
            param(
                [Parameter(Mandatory)]
                [string]$LockPath,
    ...
    It "creates a build-lock report with counts and untracked details" {
        $report = New-KitBuildLockReport -BuildLock (& $script:NewTestReportLock) -RepoRoot $script:TempRoot -WhatIf
    ...
    It "keeps missing and mismatch details and serializes JSON" {
        $missingReport = New-KitBuildLockReport -BuildLock (& $script:NewTestReportLock -Path "manifests/missing.json" -UntrackedPolicy "pass") -RepoRoot $script:TempRoot -WhatIf
        $mismatchReport = New-KitBuildLockReport -BuildLock (& $script:NewTestReportLock -Hash ("0" * 64) -UntrackedPolicy "pass") -RepoRoot $script:TempRoot -WhatIf
    ...
    It "writes explicit reports and returns expected exit codes" {
        ...
        (& $script:NewTestReportLock -Path $absoluteExistingPath -UntrackedPolicy "manual") | ConvertTo-Json -Depth 8 | Set-Content -LiteralPath $manualLockPath -Encoding UTF8
        (& $script:NewTestReportLock -Path $absoluteMissingPath -UntrackedPolicy "pass") | ConvertTo-Json -Depth 8 | Set-Content -LiteralPath $failedLockPath -Encoding UTF8

        $manualProcess = & $script:InvokeTestBuildLockProcess -LockPath $manualLockPath -ReportPath $manualReportPath
        $failedProcess = & $script:InvokeTestBuildLockProcess -LockPath $failedLockPath -ReportPath $failedReportPath
