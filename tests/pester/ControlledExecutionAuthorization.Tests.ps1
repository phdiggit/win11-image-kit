Describe "Controlled execution authorization contract" {
    ...
    function Read-AuthorizationFixture {
        param([string]$Name)
    ...
    It "passes the matched fixture without allowing true execution" {
        $result = Test-KitControlledExecutionAuthorization -InputObject (Read-AuthorizationFixture -Name "matched")
    ...
    It "blocks execute requests in the current Issue 17 stage" {
        $result = Test-KitControlledExecutionAuthorization -InputObject (Read-AuthorizationFixture -Name "execute-request-blocked")
    ...
    It "blocks missing token, missing disk identity, and stale run IDs" {
        foreach ($name in @("missing-token", "missing-disk-identity", "stale-runid")) {
            $result = Test-KitControlledExecutionAuthorization -InputObject (Read-AuthorizationFixture -Name $name)
            Assert-KitEqual $result.status "blocked"
    ...
    It "keeps token match insufficient for true execution" {
        $input = Read-AuthorizationFixture -Name "matched"
