Describe "Capability registry report" {
    ...
    It "keeps failed, manual, warning, and orphan details in the report" {
        $capabilities = @(
            [pscustomobject]@{ id = "passed"; issue = "#11"; status = "implemented"; context = "machine"; mutationLevel = "audit-only"; manifest = "manifests/context-scope.json"; schema = "schemas/context-scope.schema.json"; entrypoints = @(); validateEntrypoints = @(); tests = @("tests/pester/ContextScopeSchema.Tests.ps1"); docs = @("docs/archive/completed-roadmap/issue-10/24-issue10-context-scope-split.md"); notes = "fixture" },
            [pscustomobject]@{ id = "manual"; issue = "#11"; status = "implemented"; context = "mixed"; mutationLevel = "plan-only"; manifest = "manifests/context-scope.json"; schema = "schemas/context-scope.schema.json"; entrypoints = @(); validateEntrypoints = @(); tests = @("tests/pester/ContextScopeSchema.Tests.ps1"); docs = @("docs/archive/completed-roadmap/issue-10/24-issue10-context-scope-split.md"); notes = "fixture" },
