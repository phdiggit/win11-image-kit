Describe "Capability registry consistency" {
    ...
    It "marks real mutation, mixed context, and planned capabilities as manual instead of passed" {
        $capabilities = @(
            [pscustomobject]@{ id = "real"; issue = "#11"; status = "implemented"; context = "machine"; mutationLevel = "real-mutation"; manifest = "manifests/context-scope.json"; schema = "schemas/context-scope.schema.json"; entrypoints = @(); validateEntrypoints = @(); tests = @("tests/pester/ContextScopeSchema.Tests.ps1"); docs = @("docs/archive/completed-roadmap/issue-10/24-issue10-context-scope-split.md"); notes = "fixture" },
            [pscustomobject]@{ id = "mixed"; issue = "#11"; status = "implemented"; context = "mixed"; mutationLevel = "plan-only"; manifest = "manifests/context-scope.json"; schema = "schemas/context-scope.schema.json"; entrypoints = @(); validateEntrypoints = @(); tests = @("tests/pester/ContextScopeSchema.Tests.ps1"); docs = @("docs/archive/completed-roadmap/issue-10/24-issue10-context-scope-split.md"); notes = "fixture" },
