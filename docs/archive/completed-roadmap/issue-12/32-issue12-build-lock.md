...
本文件是 Build Lock runbook / design note。Issue #12 的后续收口材料分为：

- [Acceptance Matrix](33-issue12-build-lock-acceptance.md)：验收矩阵、非目标、PR Fast CI 边界和后续 lock 更新 checklist。
- [Close Preparation](34-issue12-close-preparation.md)：人工关闭前候选 checklist 和证据链。
- [Main Validation Evidence](35-issue12-main-validation-evidence.md)：main push / workflow_dispatch 证据脚手架；没有真实证据时保持 pending。

后续 intentional hash update 必须在 PR body 说明原因。不要把自动更新 lock 当作修复 CI 的替代方案；hash mismatch 和 watched-but-untracked warning 都必须保留为可审阅信号。
