...
| `scripts/common/` Future True UX report builders | Report-only current guardrail | Retained `New-FutureTrueUxRestore*.ps1` helpers | Keep only while they protect current authorization, dry-run, mock, no-execution, and final stop-line semantics. Preparation-only stage report builders were deleted by Issue #121. |
...
| `scripts/validate/` Future True UX validators | Report-only current guardrail | Retained `Test-FutureTrueUxRestore*.ps1` validators | Keep current gate entrypoints. Preparation-only negative-review, approval-checklist, packet-preview, and human-handoff validators were deleted by Issue #121. |
...
| `tests/fixtures/user-experience/future-true-restore/` | Report-only current guardrail | `authorization`, `current-user`, `default-user`, `offline-image`, `machine`, `review`, `mock-review`, `no-execution-readiness-audit`, `final-stop-line-handoff` | Keep current fixture families. Preparation-only intermediate fixture families were deleted by Issue #121. |
